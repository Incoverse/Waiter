...
export default class SongCMD extends WaiterCommand {
  ...
  public async exec(channel: TwitchClient, message: ChannelMessage): Promise<any> {
    ...
    if (!song || !song.item || !song.is_playing) {
      return this.bot.withChannel(channel).sendMessage(`No song is currently playing!`, { replyTo: message.message_id });
    }
    ...
    if (song.item.type == "track") {
      ...
      return this.bot.withChannel(channel).sendMessage(`Currently playing: "${songName}" by ${artistString} on ${onDevice}.`, { replyTo: message.message_id });
    }
  }
}