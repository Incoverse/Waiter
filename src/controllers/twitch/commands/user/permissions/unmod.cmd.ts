...
export default class UnmodCMD extends WaiterCommand {
  ...
  public async exec(streamer: TwitchClient, message: ChannelMessage): Promise<any> {
    ...
    if (!isMod) {
      await this.bot.channel(streamer).sendMessage(`User "${user?.display_name}" is not a moderator.`, { replyTo: message.message_id });
      return
    }
    await streamer.channel().removeMod(user.id).catch(async (err) => {
      ...
      await this.bot.channel(streamer).sendMessage(`Failed to remove "${user?.display_name}" as a moderator.`, { replyTo: message.message_id });
    })
  }
}