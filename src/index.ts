...
import { readFileSync, readdirSync, existsSync, createWriteStream, mkdirSync, unlinkSync, writeFileSync } from "fs";
...
global.inMaintenance = process.env.MAINTENANCE == "YES";
...
if (!existsSync(CONFIG_PATH)) {
  if (!existsSync("./config.template.jsonc")) {
    global.logger.error("Config file not found, and config template file is also missing. Cannot continue. Please redownload DrBot from the official source.", "DrBot-INIT");
    process.exit(1);
  } else {
    global.logger.warn("Config file not found. Creating one from the template...", "DrBot-INIT");
    const templateConfig = readFileSync("./config.template.jsonc", { encoding: "utf-8" });
...
let config = JsonCParser.parse(
  readFileSync(CONFIG_PATH, { encoding: "utf-8" })
);

const configUpdateResult = await updateConfig(CONFIG_PATH);
...
if (config.mainServer == "1234567890123456789" || config.developmentServer == "1234567890123456789") {
  global.logger.error("Please set your mainServer and developmentServer IDs in the config file before running DrBot.", "DrBot-INIT");
  process.exit(1);          
}
  ...
  client.on(Events.InteractionCreate, async (interaction: AutocompleteInteraction) => {
    ...
    if (interaction.isAutocomplete()) {
      ...
      if (global.inMaintenance) {
        return await interaction.respond([
          {
            name: "DrBot is currently in maintenance, please wait a moment and try again.",
            value: "--"
          }
        ])
      }
    ...
    if (global.inMaintenance) {
      return await interaction.reply({
        embeds: [
          new EmbedBuilder().setTitle("DrBot is currently in maintenance").setDescription(
            "The creator of DrBot is currently performing maintenance on the bot, please wait a moment and try again." 
          ).setColor(Colors.Red)
        ],
        ephemeral: true,
      });
